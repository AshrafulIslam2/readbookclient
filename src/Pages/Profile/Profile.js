...
const Profile = () => {
  const { userProfileUpdate, user } = useContext(authcontext);
      ...
      .then((Imgdata) => {
        if (Imgdata.success) {
          ...
          userProfileUpdate(updatedData)
            .then(() => {
              //update user in database for photos only
              toast.success("Profile Updated done successfully!");
              reset();
            })
            .catch((error) => {
              console.log(error);
        ...
        <div className="flex flex-col space-y-4">
          <div>
            <h2 className="text-2xl font-semibold">
              {user?.photoURL ? user.displayName : "PLease Update Name"}
            </h2>
          ...
          <div className="space-y-1">
            ...
            <span className="flex items-center space-x-2">
              ...
              <span className="dark:text-gray-400">
                {user?.photoURL ? user.phoneNumber : "PLease Provide number"}
              </span>
            </span>
          </div>
        </div>
